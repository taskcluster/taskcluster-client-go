...
type ` + api.apiDef.Name + ` tcclient.BaseClient
...
`
	...
	content += "\t" + exampleVarName + " := " + api.apiDef.Name + "(tcclient.BaseClient{\n"
...
func (entry *APIEntry) generateDirectMethod(apiName string) string {
	...
	content += queryCode
	content += "\tcd := tcclient.BaseClient(*" + entry.Parent.apiDef.ExampleVarName + ")\n"
	if entry.Output != "" {
